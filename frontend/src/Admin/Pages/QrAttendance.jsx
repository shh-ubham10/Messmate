...
const QrAttendance = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const userId = selectedId || auth.userId;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:5000/encode/id", {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-center py-3 px-2 md:py-8  md:px-4 ">
        <Autocomplete
          disablePortal
          options={id}
          sx={{ width: 300 }}
