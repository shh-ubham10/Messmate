...
export const createNewUser = asyncHandler(async (req, res) => {
  ...
  const { name, email, regId, mobileno, role, password, cpassword } = req.body;
  ...
  if (pwdIsCpwd) {
    return res
      .status(409)
      .json({ message: "Confirm Password doesnt match with password" });
  }
  ...
  if (user) {
    // Set up a folder for attendance in Firebase Realtime Database
    const db = getDatabase();
    const attendanceRef = db.ref(`Students/${regId}`);

    // Initialize the folder with an empty attendance structure
    await attendanceRef.set({
      name: name,
    });

    const bucket = firebaseAdmin.storage().bucket(); // Get the default bucket
    const fileName = `${regId}.jpeg`; // Use regId as the filename
    const fileRef = bucket.file(`Images/${fileName}`); // Specify the file path
...
export const updateUser = asyncHandler(async (req, res) => {
  ...
  // Check for duplicate
  const duplicate = await User.findOne({ email: email, mobileno: mobileno })
    .lean()
    .exec();
